    ...
    () {
      test(
        ...
        () async {
         ...
         await when(client.get(
                  Uri.parse('https://jsonplaceholder.typicode.com/albums/1')))
              .thenAnswer((realInvocation) async { 
              // var result=  http.Response('{"userId": 1, "id": 2, "title": "mock"}', 200);
              //     expect(await fetchAlbum(client), isA<Album>());
                  return http.Response('{"userId": 1, "id": 2, "title": "mock"}', 200);
  // return http.Response('{ "title": "mock"}', 200);

                  }
                        

                  );
                  // final Album myAlb= await fetchAlbum(client);

          //  expect( myAlb, isA<Album>()); 
          expect(await fetchAlbum(client), isA<Album>());
        },
      );
      test(
